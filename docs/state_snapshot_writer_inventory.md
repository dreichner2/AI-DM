...
- `serialized_metadata`: non-gameplay snapshot metadata writes that reload,
  revise, audit, and commit under the per-session turn coordinator.
- `initialization_import`: session creation/import paths that establish the
  starting runtime snapshot.
- `onboarding_initialization`: onboarding-only finalization before a new
  session is published, with any replay/repair of a visible session serialized.
...
| `aidm_server/services/campaign_pack_progress.py` | `_update_campaign_pack_progress_locked` | 2 | `campaign_pack_serialized` | Standalone updates discover and acquire the complete shared-progress session group in sorted order, discard clean pre-wait identities, revalidate membership from fresh rows, then keep every lease through commit or rollback. Transaction-joining callers may use `commit=False` only while already holding that complete lock set. | Records `session_state_mutation_audits` for migration/progress writes and progress events when changed; membership changes force a pre-mutation reacquire instead of a partial commit. | Keep behind the shared-progress coordination boundary; do not call the locked helper directly. |
| `aidm_server/services/campaign_pack_progress.py` | `_control_campaign_pack_progress_locked` | 1 | `campaign_pack_serialized` | Operator controls use the same fresh acquire/revalidate boundary and retain all shared-session leases through the durable commit. Transaction-joining callers fail closed if any group lease is missing. | Records `session_state_mutation_audits` and progress events for operator control changes. | Keep behind the shared-progress coordination boundary; do not call the locked helper directly. |
| `aidm_server/services/session_state_mutation.py` | `mutate_session_snapshot_metadata` | 1 | `serialized_metadata` | Reloads the current row after acquiring `session_turn_coordinator.serialized(session_id)` and commits before releasing the coordinator. | Increments `stateRevision`, stamps the bounded in-snapshot mutation audit, and records a durable `session_state_mutation_audits` row with a snapshot diff. | Keep as the shared boundary for metadata that is not represented by the gameplay change schema. |
...
| `aidm_server/services/play_now.py` | `_sync_play_now_snapshot` | 1 | `onboarding_initialization` | For a newly imported session, completes the uncommitted starting snapshot before the onboarding transaction is published. For an idempotent replay, `ensure_play_now_adventure` reloads under `session_turn_coordinator.serialized(session_id)` and commits before releasing it. | Uses `state_snapshot_for_session` to preserve existing runtime fields while synchronizing the selected starter hero and Play Now provenance; regression coverage verifies that only the replay path acquires the session coordinator. | Keep limited to Play Now initialization/repair; route unrelated live metadata changes through `mutate_session_snapshot_metadata`. |
...
| `aidm_server/services/campaign_pack_storage.py` | `propagate_shared_campaign_pack_progress` | 1 | `campaign_pack_serialized` | The caller must already hold the complete source-and-sibling lock set. Sibling rows and snapshots are refreshed and updated inside that single transaction; the propagation loop does not acquire nested per-session leases. | Syncs durable `CampaignPackSession` progress for sibling sessions, skips stale incoming revisions, and records `session_state_mutation_audits` with source `system.campaign_pack.shared_progress`. | Covered by shared-progress propagation, complete-lock-set, fencing, concurrent-update, and stale-revision regressions. |
