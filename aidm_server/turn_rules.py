...
def build_roll_prompt(rule_hint: RuleHint, pending_turn_id: int | None = None) -> str:
    ...
    modifier_text = (
        f' Your persisted {modifier:+d} modifier will be applied automatically.'
        if modifier not in (None, 0)
        else ''
    )
    pending_prefix = f'Resolve pending turn {pending_turn_id}: ' if pending_turn_id else ''
    return (
        f'{pending_prefix}Please roll {roll_label}{dc_hint}.{modifier_text} '
        'The server will generate and record the authoritative result.'
    )
